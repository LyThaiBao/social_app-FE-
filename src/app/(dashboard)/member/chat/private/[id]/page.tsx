...
import {File,Info,Reply, Send} from "lucide-react"
...
// -----------------------------------------------------------------------------------
  ...
  async function onSend(mess:ChatType){
    console.log(">>> ONSEND: ",replyMessage)
    ...
    return (
        ...
        return ( // ------------------------Show Message ------------------------------------
            <div key={index} className={`flex ${isMyMessage ? "justify-end" : "justify-start"}  mb-4 group `}>
                {m.parentId && (
                <div className="mb-2 p-2 border-l-4 border-blue-400 bg-gray-100 rounded-r-lg text-sm">
